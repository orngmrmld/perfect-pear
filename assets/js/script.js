...
// }
// const snackButton = document.querySelector('.restaurant-card .button.snack');
...
// Get HTML elements
const movieCard = document.querySelector(".movie-card");
const snackContainer = document.querySelector('.restaurant-card');
...
async function generateRandomMovie() {
  ...
  // Create movie card and snack HTML
  const movieCardHTML = `
  ...
  `;
  const snackHTML = `
    <p>${snack[randomSnack].name}</p>
    <img src="${snack[randomSnack].image}" alt="${snack[randomSnack].name}">
  `;
  console.log(snackHTML);
  
  // Update movie card and snack container with HTML
  movieCard.innerHTML = movieCardHTML;
  snackContainer.innerHTML = snackHTML;
  
  // Show the snack container
